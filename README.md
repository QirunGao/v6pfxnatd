...
The program does not save the previous prefix, write runtime state, create temporary rule files, or invoke external commands.
...
- Save prefix history.
- Save applied state.
- Write state under `/run`, `/tmp`, or another directory.
...
Kernel route state and the managed nftables table are the only runtime sources of truth.
...
- The nftables schema version.
- The desired fingerprint stored in `dnat_prefixes` and `snat_prefixes`.

The unchanged path must not delete or create a table, modify a map, replace a rule, create a modifying connection, or call `Flush()`.
...
At least one `[[mappings]]` entry is required. After loading, mappings are normalized into deterministic inside-prefix order. Later stages receive immutable normalized configuration.
...
Mappings are sorted deterministically. No later stage may mutate this value.

### Natural failures

TOML parse errors, invalid IPv6 prefixes, non-`/64` inside prefixes, duplicate mappings, out-of-range subnet IDs, or an invalid managed table name terminate startup. No nftables connection has been created, so the ruleset cannot have changed.
...
Return an error when the configured WAN link cannot be read, no matching default route exists, a route query fails, or no PD candidate exists. End the current `Reconcile` immediately and leave the managed table unchanged.
...
```text
dnat_prefixes: public /64 -> inside IPv6 /64
snat_prefixes: inside IPv6 /64 -> public /64
```
...
The model contains a schema version:

```go
const SchemaVersion uint32 = 2
```

Increment the schema version whenever the final netlink expression encoding changes, including map names, chain names, expression order, match conditions, hook, priority, NAT expression shape, and encoder bug fixes. Increment it even when the high-level `DesiredSpec` is unchanged. Otherwise, an incorrectly encoded old rule could retain a fingerprint that appears current.
...
The fingerprint must cover every value that changes managed behavior:
...
- Schema version and fingerprint metadata encoded in those comments.
...
Missing managed comments, a different schema version, or a different fingerprint return `false`. External changes that preserve the current managed map comments are outside this application's detection and recovery contract.
...
3. Creation of `dnat_prefixes` and `snat_prefixes` maps.
4. Creation of every map element.
...
A successful transaction means the old managed table has been replaced by the complete new table. The daemon does not save the new PD, fingerprint, generation, update time, or table handle. The next event reads kernel state again.
...
The straight-line shape is intentional. Every error before `CommitReplacement` prevents submission. There is no rollback branch, retry branch, or saved-state update.
...
The fingerprint stored in managed nftables object metadata remains kernel state, not process-local state.
...
restart the process
    ...
    -> observe a different schema version or fingerprint
...
No writable runtime directory is required. The daemon creates no PID, lock, state, or temporary rules file.
...
```text
reconcile failed: read network snapshot: delegated prefix route not found
...
- Black-box CLI/config tests plus boundary tests for configuration, derivation, fingerprinting, comparison, watcher lifecycle, and trigger coalescing.

The implementation does not invoke `ip` or `nft`, persist runtime state, actively retry reconciliation, or mutate conntrack state.
...
The current pre-release version is `0.1.0`. Production readiness still depends on validation against the target gateway kernel and nftables version.
